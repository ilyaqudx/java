var qqapiDotNet = function() {	
	...
	var result;
	var apiurl = "http://img2.sj.qq.com/api/";
	function IsIEAndNet(){
	...
	function InstallAndDown(channlID,urlParam){

			dotNETRuntimeVersion = "2.0.50727";
	      	if (HasRuntimeVersion(dotNETRuntimeVersion)){
	        	result = apiurl+"Plugin/2.0/Updatehelper2.application"
				var url = result + "?ChannelID=" + channlID + "&UrlParam=" + encodeURIComponent(urlParam);
				result = url;
	        	...
	        	if(HasRuntimeVersionBase4(dotNETRuntimeVersion)){
					result = apiurl +"Plugin/4.0/Updatehelper4.application";
					var url = result + "?ChannelID=" + channlID + "&UrlParam=" + encodeURIComponent(urlParam);
					result = url;
		...
		qqapiDotNet.prototype.exInstallAndDown = function(channlID,urlParam){

			InstallAndDown(channlID,urlParam);
		}
...
var ConfigIcon = {
	App:{
		b:"http://img2.sj.qq.com/api/images/yy.png", //大图地址
		s:"http://img2.sj.qq.com/api/images/yy-s.png"  //小图地址
	},
	Photo:{
		b:"http://img2.sj.qq.com/api/images/bz.png",
		s:"http://img2.sj.qq.com/api/images/bz-s.png"
	},
	Ebook:{
		b:"http://img2.sj.qq.com/api/images/dzs.png",
		s:"http://img2.sj.qq.com/api/images/dzs-s.png"
	},
	Video:{
		b:"http://img2.sj.qq.com/api/images/sp.png",
		s:"http://img2.sj.qq.com/api/images/sp-s.png"
	},
	Ring:{
		b:"http://img2.sj.qq.com/api/images/ls.png",
		s:"http://img2.sj.qq.com/api/images/ls-s.png"
	},
	Theme:{
		b:"http://img2.sj.qq.com/api/images/zt.png",
		s:"http://img2.sj.qq.com/api/images/zt-s.png"
	},
	Magazine:{
		b:"http://img2.sj.qq.com/api/images/zz.png",
		s:"http://img2.sj.qq.com/api/images/zz-s.png"
	}
};
    //设置图片缓存
    (function(){
    	var QQAPI_ImgsSrc=new Array();
    	    QQAPI_ImgsSrc[0]="http://img2.sj.qq.com/api/images/sjqq_sprite.png";
    	    QQAPI_ImgsSrc[1]="http://img2.sj.qq.com/api/images/sj.gif";
    	    QQAPI_ImgsSrc[2]="http://img2.sj.qq.com/api/images/sjbg.png";
...
var QQAPI_UI = {
	...
	Statistics:function(opts,flag){
		//统计数据
            var url="http://androidpc.app.qq.com/app1/channel.do?setupkey="+opts.asistanturlid+"&flag="+flag+"&callback=callback";
            try{
	        	var oldScript =window.top.document.getElementById("scrijs");       // 如果页面中注册了调用的服务器，则重新调用
            }catch(ex){
            	var oldScript =document.getElementById("scrijs");  
            }
			if(oldScript){
				try{
					window.top.document.getElementsByTagName("body")[0].removeChild(oldScript);
				}catch(ex){
					document.getElementsByTagName("body")[0].removeChild(oldScript);	
				}
			}
			try{
				var script =window.top.document.createElement("script");       // 如果未注册该服务器，则注册并请求之
			}catch(ex){
				var script =document.createElement("script");       // 如果未注册该服务器，则注册并请求之
			}
				script.setAttribute("type", "text/javascript");
				script.setAttribute("src",url);
				script.setAttribute("id", "scrijs");
			try{
	...
	UIDEFAULT: function (v, opts) {
		...
		if (typeof window.pageYOffset != 'undefined') { 
            try{
       	  		scrollTop = window.top.pageYOffset; 
		 	}catch(ex){
		...
		if (v == 0) {
			...
			if(exqqapiDotNet.exIsIEAndNet()){

				exqqapiDotNet.exInstallAndDown(opts.asistanturlid,QQApi.FormatUrl(opts));
			}else{
				inc1.style.display="none";
				inc0.style.display="block";
				top0=middle-inc0.offsetHeight/2;
				inc0.style.top=top0+"px";
			
			}
			
			QQAPI_UI.Statistics(opts,0);
	...
	initPopwinInnerHTML: function (opts) {
		...
		var _asistanturl = "http://agent.sj.qq.com/webservices/installer.do?qdh=990420";
