...
function ScrollPic(scrollContId, arrLeftId, arrRightId, dotListId) {
  ...
  this.initialize = function() {
    ...
    if (this.dotListId) {
      ...
      if (this.dotListObj) {
        var pages = Math.round(this.listDiv01.offsetWidth / this.frameWidth
            + 0.4), i, tempObj;
        for (i = 0; i < pages; i++) {
          ...
          sina.addEvent(tempObj, "click", Function("ScrollPic.childs["
              + this.ID + "].pageTo(" + i + ")"))
        }
      }
    }
  ...
  this.accountPageIndex = function() {
    this.pageIndex = Math
        .round(this.scrollContDiv.scrollLeft / this.frameWidth);
...
jQuery.fn.prepare_slider = function(fatherId,id,childTag){
  var father=jQuery('#'+fatherId);
  var me=jQuery('#'+id);
  ...
  var right_clicks = 0;
  var left_clicks = 0;
  var li_col = jQuery("#"+id+" > "+childTag);
  var li_width = li_col.outerWidth(true);
  var viewWindow = Math.round(father.width()/li_width);
  ...
  me.css('left','0px');
  me.css('width', x_pos+'px');
  
  var is_playing = false;
  var completed = function() { is_playing = false; };
  jQuery('#LeftArr').click( function(){
    cur_offset = me.position().left;
    if (!is_playing){
      if (left_clicks > 0) {
          is_playing = true; me.animate({'left': cur_offset + li_width + 'px'}, 700, "linear", completed);
          right_clicks++; 
          left_clicks--;
        } 
        else {
          is_playing = true;
          jQuery('#'+id).animate({'left':    -li_width*total_clicks + 'px'}, 700, "linear", completed);
          right_clicks = 0;
  ...
  jQuery('#RightArr').click( function(){
    if (!is_playing){     
      ...
      if (right_clicks > 0) {
          is_playing = true; 
          me.animate({'left': cur_offset - li_width + 'px'},700, "linear", completed );
          right_clicks--; left_clicks++; 
      } 
      else { 
          is_playing = true; me.animate({'left':    0 + 'px'},700, "linear", completed ); 
          left_clicks = 0;
