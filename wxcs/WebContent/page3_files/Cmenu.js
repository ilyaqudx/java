...
$.fn.extend({
	...
	seleFn:function(){
		$(this).each(function(){
			...
			var _method=$(this),_parent=null;
				...
				w = $(this).width()+parseInt($(this).css('padding-right'))+parseInt($(this).css('padding-left'))+parseInt($(this).css('border-right-width'))+parseInt($(this).css('border-left-width')),
				stype= ($(this).attr('stype')=='liandong')?"stype='liandong'":"",
			    border  = "<span class='apps_fn_seleJs'><span class=\"seleTB\" "+stype+"></span></span>";
			var	rect='2px '+(_method.width()-23)+'px 23px 2px';
			w = !!window.ActiveXObject?w+1:w;
			_method.css({'height':'25px','line-height':'24px','clip':'rect('+rect+')','position':'absolute','background':'none','top':_method.position().top+3,'left':_method.position().left}).before(border);
			_method.prev().find('.seleTB').width(w-26);
			_method.attr('jsSim','true');
		})//end each
		...
		_ld.bind('change',function(){
			...
			if(""==tvalue){
				for(var i=ldindex+1;i<ldlen;i++){ 
					...
					_ld.eq(i).prev('.apps_fn_seleJs').find('.seleTB');
				}
				return false;
			}
			$.ajax({
	...
	fileTxFn:function(){
		$(this).each(function(){
			...
			$(this).height(0).width(0).before("<span class='apps_fn_fileCss'><input type='text' class='apps_hdlt apps_w180' value='"+$(this).val()+"' readonly='readonly' /> <input class='apps_btn3 apps_w100' type='button' value='上传'></span>");
